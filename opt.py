...
def LVST(Co, regs):
    ...
    beta1 = regs['beta1']
    beta2 = regs['beta2']
    S_hat = cp.Variable((O, O),symmetric=True)
    P_hat = cp.Variable((O, O))

    contraints = [S_hat >= 0, cp.sum(S_hat[:,0]) == 1, cp.diag(S_hat) == 0]

    obj = rho1*cp.norm(S_hat, 1) + beta1*cp.norm(P_hat, 'nuc') + beta2*cp.norm(Co@S_hat+P_hat-S_hat@Co-P_hat.T,'fro') 
...
def GST(Cos, regs):
    ...
    constraints = []
    for k in range(K):
        S_hat.append(cp.Variable((O, O),symmetric = True))
        constraints += [S_hat[k] >= 0]
        constraints += [cp.diag(S_hat[k]) <= 1e-6]
    
    constraints += [cp.sum(S_hat[0][1:,0]) == 1]
    
    obj = 0
    for k in range(K):
        obj += cp.norm(S_hat[k]@Cos[k,:,:]-Cos[k,:,:]@S_hat[k]) \
            + rho1*cp.norm(S_hat[k], 1)

        for l in range(k):
            obj += rho2*cp.norm(S_hat[k] - S_hat[l], 1)
